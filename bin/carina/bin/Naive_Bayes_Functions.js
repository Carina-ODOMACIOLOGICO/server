...
class NaiveBayesFunctions {
  ...
  saveLabelsInDatabase(data, callback) {
    ...
    async.eachOfSeries(
      ...
      (label, i, cbLabel) => {
        let id = new mongoose.Types.ObjectId();
        let id_label = mongoose.mongo.ObjectId(label);
        newLabel.push({
          _id: id,
          name: id_label,
          isA: "colDataset",
            ...
            Labels.find({
              ...
              if (l.length === 0) {
                Labels.create(label, function (err, lb) {
                  ...
                  socket.emit("partialProgress", { valuePartial });
                  console.log(`Label ${valuePartial}%`);
                  cbLabel();
                });
              } else {
                cbLabel();
    ...
    async.eachOfSeries(
      ...
      (mt, i, cbmt) => {
        if (mt.label != undefined && _.find(idsTokens, { name: mt.token }) != undefined) {
          console.log(`DataCell: ${learningDatasetId} ${mt.token} ${mt.label}`);
          CellsDataset.create(
            {
              isA: "CellDataset",
              learningDatasetId: learningDatasetId,
            ...
            function (err, cd) {
              ...
              socket.emit("partialProgress", { valuePartial });
              console.log(`saveWordsLabelsInDatabase ${valuePartial}%`);
              cbmt();
            }
          );
        } else {
      ...
      (err) => {
        ...
        console.log("Palabras guardadas en la base de datos");
        callback(null, data);
      }
    );
  }
  ...
  /*getProbabilities(data, callback) {
    ...
    console.log(`labelsTest ${labelsTest} ${documentsTest}`);
  ...
  initCleanInterview(data, callback) {
    ...
    //console.log(`interview: ${JSON.stringify(data.interview, null, 2)}`);
    callback(null, data);
  }

  getProbabilities(data, callback) {
    let { dataset, interview, model } = data;
    ...
    Labels.find({ learningDatasetId: dataset._id }).exec((err, labels) => {
      ...
      // let labels_id = labels.map((l) => mongoose.Types.ObjectId(l._id));
      let labels_name = labels.map((l) => mongoose.Types.ObjectId(l.name));
      ...
      Belief.find({ name: { $in: tokens } })
        ...
        .exec((err, beliefs) => {
          ...
          // console.log("datos:", beliefs_id, tokens, labels_name);
          CellsDataset.find({
            learningDatasetId: dataset._id,
            "has.head": { $in: labels_name },
            "has.field": { $in: beliefs_id },
          })
            /*.populate({
              path: "has.field", //61e59fc06e608d6e22686222
              model: model,
              //select: "name",
            }) 
            .populate({
              path: "has.head", //61e59f8b6e608d6e226861b4
              //select: "name",
            })*/
            .exec(async (err, celldata) => {
              if (err) throw err;
              // console.log(`celldata ${JSON.stringify(celldata)}`);
              let dataByLabel = await _.groupBy(celldata, "has.head");
              //console.log(`dataByLabel ${JSON.stringify(dataByLabel)}`);
              let labelProbability = [];
              for (let l in dataByLabel) {
                //console.log(l);
                // await console.log(l, await _.find(labels, (lb) => lb.name == l).has.probability); //l.name === l).has.probability);
                let pM = await _.find(labels, (lb) => lb.name == l).has.probability;
